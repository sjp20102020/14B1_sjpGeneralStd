# The Tools Used in `angular-enterprise-kickstart`

## Introduction

`angular-enterprise-kickstart` is standards-based, so it uses all the usual tools to manage
...
project makes use and how they fit in to the `angular-enterprise-kickstart` picture.

## Git

[Git](http://git-scm.com/) is a distributed version control system.
`angular-enterprise-kickstart` uses git to manage its codebase. While in theory you don't have
to use Git once you download `angular-enterprise-kickstart`, this project makes the assumption
...
While `angular-enterprise-kickstart` makes heavy use of Node.js behind the scenes, you as the
...
you first start with `angular-enterprise-kickstart` you have to tell NPM to install the
...
Don't worry about knowing Node.js in order to use `angular-enterprise-kickstart`; Grunt is
...
to get started with `angular-enterprise-kickstart`. But for those curious or looking to go a
...
Grunt is the engine behind `angular-enterprise-kickstart`. It's the magic that makes it move.
...
`angular-enterprise-kickstart` comes with a `bower.json` file that looks something like this:

```js
{
  "name": "angular-enterprise-kickstart",
...
in `angular-enterprise-kickstart` by default in there to see what I mean.
